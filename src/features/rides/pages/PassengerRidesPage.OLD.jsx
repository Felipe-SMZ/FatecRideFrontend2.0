...
export function PassengerRidesPage() {
  ...
  useEffect(() => {
  console.log('\n🔄 useEffect PassengerRidesPage executado');
    console.log('📊 Estado:', {isPassenger, isBoth, isDriver, userTipo: user?.tipo});
    
    // Apenas buscar se for passageiro ou ambos
    if (isPassenger || isBoth) {
      console.log('✅ Usuário é PASSAGEIRO ou AMBOS - Buscando solicitações...');
      fetchMyRequests();
    } else if (isDriver) {
      console.log('ℹ️ Usuário é MOTORISTA puro - Não busca solicitações');
      setLoading(false);
      setRequests([]);
    } else if (!user?.tipo) {
      console.log('⚠️ Tipo de usuário não definido - aguardando...');
      setLoading(false);
    } else {
      console.log('❓ Tipo de usuário desconhecido:', user?.tipo);
      setLoading(false);
    }
  }, [user?.tipo, user?.id]); // Apenas re-executar se tipo ou ID mudar
  ...
  const fetchMyRequests = async () => {
    try {
      ...
      // Buscar TODAS as solicitações (concluídas inclui aceitas e pendentes)
      const response = await fetch('http://localhost:8080/solicitacao/concluidas', {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('📋 Resposta do backend (todas solicitações):', data);
        ...
        // Não filtrar por status - mostrar todas com badge de status
        setRequests(requestsArray);
        console.log('✅ Total de solicitações:', requestsArray.length);
          ...
          try {
            ...
            if (errorData.message?.includes('Nenhuma solicitação')) {
              console.log('ℹ️ Nenhuma solicitação pendente (backend retornou 500)');
              setRequests([]);
            } else {
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-3 border-t flex gap-2">
                      ...
                      {isActive && (
                        <Button
                          onClick={() => setOpenChat({
                            ...
                            otherUserName: request.motorista?.nome || 'Motorista'
                          })}
                          className="bg-fatecride-blue hover:bg-fatecride-blue-dark flex-1 sm:flex-none"
                          size="sm"
                        >
                      ...
                      {canCancel && (
                        ...
                        >
                          {cancelingId === request.id_solicitacao ? 'Cancelando...' : 'Cancelar'}
                        </Button>
                      )}
                    </div>
                  </div>
